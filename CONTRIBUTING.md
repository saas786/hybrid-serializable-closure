...
The code for the package is handled via its [GitHub Repository](https://github.com/themehybrid/hybrid-serializable-closure).  You can open tickets, create patches, and send pull requests there.
...
Hybrid Serializable Closure package is developed with the WordPress.org [theme review guidelines](https://make.wordpress.org/themes/handbook/review) in mind.  While the package is not a theme in and of itself, many theme authors use it within their themes on the official theme repository.  These guidelines should sufficiently cover any other marketplace or theme shop rules for submission.  All new features and code changes must abide by these guidelines to make it into the project.

## Coding standards

Hybrid Serializable Closure follows a mixture of the following standards.
...
Any code contributed to Hybrid Serializable Closure via patches, pull requests, or other means will be licensed under the [GPL version 2](https://www.gnu.org/licenses/old-licenses/gpl-2.0.html) or later.  By contributing code to the project, you provide consent to use such code under this license.  The exception to this rule is when bringing in third-party code with an alternate open source license.

## Versioning

Hybrid Serializable Closure uses [semantic versioning](https://semver.org).  Version numbers will look like `3.2.1` where `3` is the "major" release, `2` is the minor release, and `1` is the patch release.
