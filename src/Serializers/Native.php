...
namespace Hybrid\SerializableClosure\Serializers;

use Closure;
use DateTimeInterface;
use Hybrid\SerializableClosure\Contracts\Serializable;
use Hybrid\SerializableClosure\SerializableClosure;
use Hybrid\SerializableClosure\Support\ClosureScope;
use Hybrid\SerializableClosure\Support\ClosureStream;
use Hybrid\SerializableClosure\Support\ReflectionClosure;
use Hybrid\SerializableClosure\Support\SelfReference;
use Hybrid\SerializableClosure\UnsignedSerializableClosure;
...
class Native implements Serializable {
     ...
     *
     * @var \Hybrid\SerializableClosure\Support\ReflectionClosure|null
     */
    protected $reflector;
     ...
     * The closure's scope.
     *
     * @var \Hybrid\SerializableClosure\Support\ClosureScope|null
     */
    protected $scope;
     ...
     */
    const string ARRAY_RECURSIVE_KEY = 'HYBRID_CORE_SERIALIZABLE_RECURSIVE_KEY';
    ...
    public function __serialize() {
        ...
        if ( $scope = $reflector->getClosureScopeClass() ) {
            $scope = ! $scope->isAnonymous() || $reflector->isBindingRequired() || $reflector->isScopeRequired()
                ? $scope->name
                : null;
        }
     ...
     *
     * @param mixed                                            $data
     * @param \Hybrid\SerializableClosure\Support\ClosureScope $storage
     *
     * @return void
     */
    public static function wrapClosures( &$data, $storage ) {
        if ( $data instanceof Closure ) {
            ...
            foreach ( $data as $key => &$value ) {
                if ( self::ARRAY_RECURSIVE_KEY === $key ) {
                    continue;
                }
            ...
            do {
                ...
                foreach ( $reflection->getProperties() as $property ) {
                    if (
                        ...
                        || static::isVirtualProperty( $property )
                    ) {
                        continue;
                    }
     ...
     *
     * @return \Hybrid\SerializableClosure\Support\ReflectionClosure
     */
            ...
            do {
                ...
                foreach ( $reflection->getProperties() as $property ) {
                    ...
                    if (
                        ...
                        || (
                            $property->isReadOnly()
                            && $property->class !== $reflection->name
                        )
                    ) {
                        continue;
                    }
